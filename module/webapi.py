...
class WebApiServer(object):
    def __init__(self):
        self.func_list = {r"|/|/help": self.showHelp}
        self.url_prefix = ''

    def delegateApi(self, environ, start_response):
        ...
        for pattern in self.func_list:
            r = re.compile(pattern)
            m = r.fullmatch(method)
            if m is not None:
                start_response('200 OK', headers)
                result = self.func_list[pattern](query, environ, m.groups())
                return [json.dumps(result).encode('utf-8')]

        start_response('404 Not found', headers)
        return ['404 Not found'.encode("utf-8")]

## this method doesn't work; result is not JSON
    def showHelp(self, query_param, environ, m):
        ...
        result = io.StringIO()
        for api in self.func_list:
            print('- ' + api, file=result, end="")
            print(textwrap.dedent(self.func_list[api].__doc__), file=result)
        return result

    def startServer(self, port, url_prefix):
        logger.debug('start server: port=%d, url_prefix=%s', port, url_prefix)
        self.url_prefix = url_prefix

        server = make_server('', port, self.delegateApi)
